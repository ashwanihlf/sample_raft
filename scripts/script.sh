...
echo
echo "Sample RAFT Application Script."
echo
...
verifyResult () {
	if [ $1 -ne 0 ] ; then
		echo "Trace "$2" "
    echo "ERROR - FAILED to execute 3 Peer End to End Application."
		echo
   		exit 1
	fi
}
...
instantiateChaincode () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		peer chaincode instantiate -o orderer.example.com:7050 -C $CHANNEL_NAME -n samplecc -v 1.0 -c '{"Args":[""]}' -P " OR('Org1MSP.member','Org2MSP.member')" >&instantiatelog.txt

		#		peer chaincode instantiate -o orderer.example.com:7050 --tls true --cafile /opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem -C $CHANNEL_NAME -n samplecc -v 1.0 -c '{"Args":[""]}' -P " OR('Org1MSP.member','Org2MSP.member')" >&instantiatelog.txt
	else
		 peer chaincode instantiate -o orderer.example.com:7050 --tls true --cafile /opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem -C $CHANNEL_NAME -n samplecc -v 1.0 -c '{"Args":[""]}' -P " OR('Org1MSP.member','Org2MSP.member')" >&instantiatelog.txt

#peer chaincode instantiate -o orderer.example.com:7050 --tls true --cafile $ORDERER_CA -C mychannel -n samplecc -v 1.0 -c '{"Args":[""]}' -P " OR('Org1MSP.member','Org2MSP.member')" >&instantiatelog.txt
	fi
	res=$?
